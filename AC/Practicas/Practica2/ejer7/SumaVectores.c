...
int main(int argc, char** argv){ 
  ...
  printf("Inicialización del vector: \n");
  #pragma omp parallel
  {
    #pragma omp for
	  for(i=0; i<N; i++){
      printf("thread %d ejecuta la iteracion del bucle %d\n",
  ...
  double tiempo_ini = omp_get_wtime();
  ...
  #pragma omp parallel
  {
	  ...
	  for(i=0; i<N; i++){
      v3[i] = v1[i] + v2[i];
      printf("thread %d (iteración del bucle %d ) ejecuta %f + %f = %f\n",
      omp_get_thread_num(),i,v1[i],v2[i],v3[i]);
	  }
  }
  ...
  if (N<10) {
    ...
    printf("Tiempo:%11.9f\t / Tamaño Vectores:%u\t/ V1[0]+V2[0]=V3[0](%8.6f+%8.6f=%8.6f) / / V1[%d]+V2[%d]=V3[%d](%8.6f+%8.6f=%8.6f) /\n",
           tiempo_fin-tiempo_ini,N,v1[0],v2[0],v3[0],N-1,N-1,N-1,v1[N-1],v2[N-1],v3[N-1]); 
