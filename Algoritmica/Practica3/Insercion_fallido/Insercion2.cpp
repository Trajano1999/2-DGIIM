...
bool todosFalse(vector<pair<bool, pair<double,double> > > v){
...
void introducir3vertices(vector<int>& res, vector<pair<bool, pair<double,double> > >& v){
    ...
    for(int i=0; i<v.size(); ++i){
        if(v[i].second.second >= max_norte && i != pos_este && i != pos_oeste){
...
vector<int> insercion(vector<pair<bool, pair<double,double> > > v){
    ...
    while(!todosFalse(v)){
        ...
        //cerr << "Insertado: " << insertado_final  << endl;
...
int main(int narg, char* argv[]){
    ...
    for(int i=0; i<insercion1.size(); ++i)
        fsalida << insercion1[i]+1 << "\n";

    //Calculo de la suma total de cercanias1
    fsalida << "\nLa suma total es: " << calculoSumaTotal(insercion1, ciudades);
